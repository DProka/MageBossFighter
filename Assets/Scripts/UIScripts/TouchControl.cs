...
public class TouchControl : MonoBehaviour, IBeginDragHandler, IDragHandler
{
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        ...
        if ((Mathf.Abs(eventData.delta.x)) > (Mathf.Abs(eventData.delta.y)) && GameController.gameController.arenaIsActive)
        {
            ...
            if(!player.movementScript.isMoving)
            {
                if (eventData.delta.x < 0)
                {
                    ...
                    if (player.movementScript.nextPoint >= GameController.gameController.points.Length)
                ...
                else
                {
                    ...
                    if (player.movementScript.nextPoint < 0)
                        player.movementScript.nextPoint = GameController.gameController.points.Length - 1;
