...
public class BlockMovepoints : MonoBehaviour
{
    ...
    void ChoosePoint(int playerPoint)
    {
        ...
        {
            GameController.gameController.points[random].GetNewStatus(Status.Blocked);
            return;
        ...
        else
        {
            for (int i = 0; i < GameController.gameController.points.Length; i++)
            {
                ...
                {
                    GameController.gameController.points[i].GetNewStatus(Status.Blocked);
                    return;
                }
            }
        }
    ...
    {
        int point1 = point;
        if (point1 >= GameController.gameController.points.Length)
            point1 = 0;
        else if (point1 < 0)
            point1 = 11;
